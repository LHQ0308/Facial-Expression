...
def loadTrainData(filename="data.pkl",mean_filename="mean.pkl",scale=1.000):
    print "loading data..."
    file=open(filename,"rb")
    mfile=open(mean_filename,"rb")
    ...
    def sharedDataSet(data,borrow=True): 
        ...
        sharedX=theano.shared(numpy.asarray((dataX-xMean)/scale,dtype=theano.config.floatX),borrow=True)
    ...
    return rval
def loadTestData(filename="data.pkl",mean_filename="mean.pkl",scale=1.000):
    file=open(filename,"rb")
    mfile=open(mean_filename,"rb")
    xMean=cPickle.load(mfile)
