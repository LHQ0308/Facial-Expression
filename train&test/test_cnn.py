...
test_hit=0
test_count=0
def test_single(filename,mean_filename,y,scale=1.000):
...
def test_knn(filename,mean_filename,y,scale=1.000):
    size,dataSet=loadTestData(filename,mean_filename,scale)
...
if __name__ == '__main__':
    ...
    Test_KNN.add(DropoutLayer(0.5))
    Test_KNN.add(SoftmaxLayer(neure[3],5,'Gaussian',0.1))
    Test_KNN.build_test_fn()
    Test_KNN.load_params('params/CNN128_ROI.pkl')
